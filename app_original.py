...
# ─── CONFIG ───────────────────────────────────────────────
CSV_PATH        = r"C:\Users\salma\Documents\collage\IR (2)\medicine_cleaned133(in).csv"
TEMPLATE_FOLDER = r"C:\Users\salma\Documents\collage\IR (2)\html_files"
CACHE_FILE      = r"C:\Users\salma\Documents\collage\IR (2)\cache.pkl"
USE_CSV         = True
app    = Flask(__name__, template_folder=TEMPLATE_FOLDER)
GOOGLE_API_KEY = "AIzaSyCBic9jEwXsuvj1m5Qyw5mmRdTt4eIp-MIx"
genai.configure(api_key=os.getenv("AIzaSyCBic9jEwXsuvj1m5Qyw5mmRdTt4eIp-MI"))
...
def search(query, docs, index, vectorizer, matrix, doc_ids, texts):
    ...
    # O(1) dict lookup instead of O(n) list.index()
    ...
    results = sorted(
        ...
        key=lambda x: x[2],
        reverse=True
    )[:20]   # limit to top 20 results for speed
    return results
...
def chat_with_gemini(user_message, conversation_history):
    # Step 1: search local CSV for relevant medicines
    csv_results = search(
    ...
    # Step 4: merge conversation history into plain text
    history_text = "\n".join(
        [f"{m['role']}: {m['content']}" for m in conversation_history]
    )

    # Step 5: call Gemini with plain text parts
    ...
    # Step 6: return assistant's reply
...
# Fast O(1) lookup dict: doc_id -> index in doc_ids list
    ...
    if request.method == "POST":
        ...
        if "query" in request.form:
            ...
            results = search(query, documents, inverted_index,
                             vectorizer, matrix, doc_ids, texts) #doing search
            results = results[:20]  # show top 20 max
